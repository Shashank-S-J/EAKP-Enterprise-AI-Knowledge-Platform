...
export const useChatStore = create((set, get) => ({
    ...
    loadConversations: async () => {
        try { const list = await convApi.list(); set({ conversations: list }); } catch { /* surfaced by toast elsewhere */ }
    },
    loadMessages: async (convId) => {
        try {
            const msgs = await convApi.messages(convId);
            set({ messages: msgs, activeConversationId: convId });
            return { ok: true };
        } catch (err) {
            // 403/404 — the user typed a conversation ID they don't own or that doesn't exist.
