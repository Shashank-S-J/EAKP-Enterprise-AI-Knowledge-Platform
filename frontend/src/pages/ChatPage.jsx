...
export default function ChatPage() {
    ...
    // Validate conversationId from URL — must be a UUID-shaped string.
    // If the user manually edits the URL to garbage, redirect to /chat.
    useEffect(() => {
    ...
    useEffect(() => {
        if (conversationId && conversationId !== activeConversationId) {
            loadMessages(conversationId).then((res) => {
                if (res && res.ok === false) {
                    // Forbidden / not-found / unknown UUID — send the user back to /chat.
                    navigate('/chat', { replace: true });
                }
            });
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps -- store actions are stable; only these reactive values matter
    ...
    const submitTitle = async () => {
        ...
        try { await convApi.rename(activeConversationId, trimmed); } catch { /* optimistic — server failure surfaces on next refresh */ }
    };
